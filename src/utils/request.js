...
request.interceptors.response.use(function (response) {
  ...
  if (error.response) {
    ...
    let errorMessage = ''
    if (status === 400) {
      console.log("===!!!!========="+store.state.user.token_refresh);
      errorMessage = '请求参数错误'
    } else if (status === 401) {
      if (!store.state.user) {
        redirectLogin()
      ...
      return request({
        method: 'POST',
        url: 'http://localhost:7788/Glogin/login/refresh',
        data: qs.stringify({
          refreshtoken: store.state.user.token_refresh,
        ...
        store.commit('setUser', JSON.stringify(res.data.data))
